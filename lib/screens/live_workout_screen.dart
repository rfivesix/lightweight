...
class _LiveWorkoutScreenState extends State<LiveWorkoutScreen> {
  ...
  Duration get _workoutDuration {
    final start = widget.workoutLog.startTime;
    final end = DateTime.now();
    return end.difference(start);
  }

  int get _totalSets => WorkoutSessionManager().completedSets.length;

  double get _totalVolume {
    double volume = 0;
    for (var re in _liveExercises) {
      for (var template in re.setTemplates) {
        if (WorkoutSessionManager().completedSets.contains(template.id)) {
          final weight = double.tryParse(_weightControllers[template.id!]?.text ?? '0') ?? 0;
          final reps = int.tryParse(_repsControllers[template.id!]?.text ?? '0') ?? 0;
          volume += weight * reps;
        }
      }
    }
  ...
  void initState() {
    ...
    if (widget.routine != null) {
      // Neues Workout mit Routine gestartet
      _liveExercises = widget.routine!.exercises.map((re) {
        return RoutineExercise(
          id: re.id,
          exercise: re.exercise,
          setTemplates: re.setTemplates.map((st) => SetTemplate.fromMap(st.toMap())).toList(),
          pauseSeconds: re.pauseSeconds,
        );
      }).toList();

      WorkoutSessionManager().startWorkout(widget.workoutLog, _liveExercises);
    } else {
      // Bereits laufendes Workout wieder öffnen
      _liveExercises = WorkoutSessionManager().exercises;
    }
    ...
    return AnimatedBuilder(
        ...
        builder: (context, _) {
          return Scaffold(
            appBar: AppBar(
              title:
                  Text(widget.workoutLog.routineName ?? l10n.freeWorkoutTitle),
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          _buildInfoItem("⏱ Dauer", 
                            "${_workoutDuration.inMinutes}:${(_workoutDuration.inSeconds % 60).toString().padLeft(2, '0')}"),
                          _buildInfoItem("💪 Volumen", "${_totalVolume.toStringAsFixed(0)} kg"),
                          _buildInfoItem("📊 Sätze", "$_totalSets"),
                        ],
                      ),
                    ),
                    Expanded(
                            ...
                            for (int index = 0;
                              ...
                              Card(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    // 🔹 Kopfzeile mit Pause-Button, Delete-Button und Drag-Handle
                                    ListTile(
                                      title: Text(
                                          ...
                                          ? Padding(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  const Divider(),
                                                  Column(
                                                    children: _liveExercises[index].setTemplates.map((setTemplate) {
                                                      final lastPerf = _lastPerformances[_liveExercises[index].exercise.nameEn];
                                                      final isCompleted = WorkoutSessionManager().completedSets.contains(setTemplate.id!);

                                                      // Working-Set-Index berechnen
                                                      int workingSetIndex = 0;
                                                      for (final st in _liveExercises[index].setTemplates) {
                                                      ...
                                                      return _buildSetRow(
                                                        workingSetIndex, // ⬅️ Hier übergeben
                                                        _liveExercises[index],
                                                        setTemplate,
                                                        isCompleted,
                                                        colorScheme,
            ...
            bottomNavigationBar: AnimatedBuilder(
              ...
              builder: (context, _) {
                ...
                // Wenn nix zu zeigen ist, gib eine leere Box zurück (kein Platzverbrauch)
